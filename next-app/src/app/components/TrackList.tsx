      ...
      <div className="flex flex-col p-2 space-y-2 w-full">
        {tracks.map((track, index) => (
          <div className="flex flex-row" key={index}>
            <Tooltip
              content={<p className="text-white">Click to view on Spotify</p>}
              placement="top-end"
            >
              <Card
                className="bg-white flex flex-row px-4 py-2 justify-between w-full"
                isPressable
                onPress={() => window.open(track.spotify_url, "_blank")}
              >
